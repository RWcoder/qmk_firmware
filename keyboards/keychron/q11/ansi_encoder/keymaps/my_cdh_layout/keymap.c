...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    [BASE] = LAYOUT_91_ansi(
        ...
        _______,  KC_TAB,   KC_Q,     KC_W,     KC_F,       KC_P,         KC_B,     KC_J,     KC_L,     KC_U,     KC_Y,     KC_SLSH,     KC_LBRC,  KC_RBRC,  KC_F9,        KC_HOME,
        ...
        _______,  KC_LCTL,  KC_LALT,  KC_LGUI,  A(KC_SPC),  MO(CONTROL),  KC_SPC,   KC_RGUI,  KC_RALT,  KC_RCTL,  KC_F8,    KC_F11,      KC_F10),
    ...
    [CONTROL] = LAYOUT_91_ansi(
        ...
        _______,  QK_MACRO_4,  _______,     KC_SLEP,     _______,    _______,       C(KC_V),  _______,  _______,  _______,    _______,    _______,     _______),
    ...
    [QWERTY] = LAYOUT_91_ansi(
        ...
        TO(BASE),  KC_LCTL,  KC_LALT,  KC_LGUI,  _______,  KC_SPC,  KC_SPC,  KC_LGUI,  KC_LALT,  KC_LCTL,  KC_LEFT,  KC_DOWN,  KC_RGHT),
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    if (record->event.pressed) {
        switch (keycode) {
            case QK_MACRO_0:
                SEND_STRING(SS_DOWN(X_LCTL)SS_TAP(X_LALT)SS_UP(X_LCTL)SS_DOWN(X_LALT)SS_TAP(X_O)SS_UP(X_LALT)"dd"SS_TAP(X_ENT));
                return false;
            case QK_MACRO_1:
                SEND_STRING(SS_DOWN(X_LGUI)SS_TAP(X_LEFT)SS_UP(X_LGUI)SS_DOWN(X_LGUI)SS_TAP(X_LEFT)SS_UP(X_LGUI));
                return false;
            case QK_MACRO_2:
                SEND_STRING(SS_DOWN(X_LSFT)SS_TAP(X_W)SS_UP(X_LSFT)SS_DELAY(300)SS_DOWN(X_LGUI)SS_TAP(X_UP)SS_UP(X_LGUI));
                return false;
            case QK_MACRO_3:
                SEND_STRING(SS_DOWN(X_LCTL)SS_TAP(X_LALT)SS_UP(X_LCTL)SS_DOWN(X_LGUI)SS_TAP(X_2)SS_UP(X_LGUI));
                return false;
            case QK_MACRO_4:
                SEND_STRING(SS_DOWN(X_LCTL)SS_DOWN(X_LALT)SS_TAP(X_DEL)SS_UP(X_LALT)SS_UP(X_LCTL)SS_DELAY(600)SS_TAP(X_DOWN)SS_TAP(X_ENT));
                return false;
            case QK_MACRO_5:
                SEND_STRING(" = ");
                return false;
